...
def save_model(model):
        torch.save(model.state_dict(), 'checkpoints/SCLGPA.pth')
...
if __name__ == "__main__":
    ...
    masks_dir=weak_path + '/' + 'masks_cl/',
    probability_dir=weak_path + '/' + p,
    augumentation=transformer_img()
    )
    ...
    Epochs:          {epochs}
    Batch size(labeled, weak_labeled):      {batch_size, batch_size}
    ...
    model_test.load_state_dict(torch.load('checkpoints/SCLGPA.pth', map_location='cpu'))
    ...
    with open('SCLGPA.txt', 'a') as file:
        # 写入文本内容，这将添加到文件的末尾
